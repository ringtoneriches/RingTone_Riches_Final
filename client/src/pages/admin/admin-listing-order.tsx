import AdminLayout from "@/components/admin/admin-layout";
import { useMutation, useQuery } from "@tanstack/react-query";
import { ListOrdered } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { apiRequest, queryClient } from "@/lib/queryClient";
import { Competition } from "@shared/schema";
import { isCompetitionLive } from "@shared/competition-config";
import { Input } from "@/components/ui/input";
import { getCompetitionTypeConfig } from "@/lib/competition-display";

export default function AdminListingOrder() {
  const { toast } = useToast();
  const { data: competitions = [], isLoading } = useQuery<Competition[]>({
    queryKey: ["/api/admin/competitions"],
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, displayOrder }: { id: string; displayOrder: number }) => {
      const res = await apiRequest(`/api/admin/competitions/${id}/display-order`, "PATCH", {
        displayOrder,
      });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/competitions"] });
      queryClient.invalidateQueries({ queryKey: ["/api/competitions"] });
      toast({ title: "Listing order updated" });
    },
    onError: () => {
      toast({ title: "Could not update listing order", variant: "destructive" });
    },
  });

  const active = competitions.filter(isCompetitionLive).sort((a, b) => (a.displayOrder ?? 999) - (b.displayOrder ?? 999));

  const save = (id: string, current: number, nextRaw: string) => {
    const next = Math.floor(Number(nextRaw));
    if (!Number.isInteger(next) || next < 0 || next > 9999) {
      toast({ title: "Use a whole number from 0 to 9999 (lower = first)", variant: "destructive" });
      return;
    }
    if (next === current) return;
    updateMutation.mutate({ id, displayOrder: next });
  };

  return (
    <AdminLayout>
      <div className="space-y-6 p-4 sm:p-6">
        <div>
          <h1 className="text-2xl font-bold">Homepage listing order</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Controls the order of competition cards below the featured slider on the home page. Lower
            numbers appear first. Featured slider slots are set separately on Featured slider.
          </p>
        </div>

        {isLoading ? (
          <p className="text-sm text-muted-foreground">Loading competitions…</p>
        ) : (
          <div className="overflow-hidden rounded-xl border bg-card">
            <table className="w-full text-left text-sm">
              <thead className="border-b bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="px-4 py-3 font-semibold">#</th>
                  <th className="px-4 py-3 font-semibold">Game</th>
                  <th className="px-4 py-3 font-semibold">Type</th>
                  <th className="px-4 py-3 font-semibold">Display order</th>
                </tr>
              </thead>
              <tbody>
                {active.map((competition, index) => {
                  const typeCfg = getCompetitionTypeConfig(competition.type);
                  const current = competition.displayOrder ?? 999;
                  return (
                    <tr key={competition.id} className="border-b last:border-0">
                      <td className="px-4 py-3 text-muted-foreground">{index + 1}</td>
                      <td className="px-4 py-3">
                        <p className="line-clamp-2 font-medium">{competition.title}</p>
                      </td>
                      <td className="px-4 py-3 text-xs uppercase text-muted-foreground">
                        {typeCfg.label}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <ListOrdered className="h-3.5 w-3.5 text-muted-foreground" />
                          <Input
                            type="number"
                            min={0}
                            max={9999}
                            defaultValue={current}
                            className="h-9 w-24"
                            data-testid={`input-listing-order-${competition.id}`}
                            onBlur={(e) => save(competition.id, current, e.target.value)}
                            onKeyDown={(e) => {
                              if (e.key !== "Enter") return;
                              save(competition.id, current, (e.target as HTMLInputElement).value);
                            }}
                          />
                        </div>
                      </td>
                    </tr>
                  );
                })}
              </tbody>
            </table>
            {active.length === 0 && (
              <p className="px-4 py-8 text-center text-sm text-muted-foreground">
                No live competitions.
              </p>
            )}
          </div>
        )}
      </div>
    </AdminLayout>
  );
}
