...
function CompetitionForm({
  ...
  const [form, setForm] = useState<CompetitionFormData>({
    ...
    badgeLabel: data?.badgeLabel || getDefaultBadgeLabel("plinko"),
    ...
    <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1 sm:pr-2">
      ...
      <div>
        ...
        <div className="w-full p-2 border border-border rounded-md bg-muted text-foreground">
          Ringtone Plinko Game
        </div>
        <p className="text-xs text-muted-foreground mt-1">
          Uses global plinko game prize configuration
        </p>
      </div>
...
export default function AdminPlinkoBalloon() {
  const { toast } = useToast();
  ...
  const competitions = allCompetitions?.filter((c) => c.type === "plinko") || [];
  ...
  const createMutation = useMutation({
    mutationFn: async (formData: CompetitionFormData) => {
      const payload: any = {
        ...
        badgeLabel: serializeBadgeLabel(formData.badgeLabel, "plinko"),
    ...
    onSuccess: () => {
      ...
      toast({ title: "Plinko game created successfully" });
    },
    onError: (error: any) => {
      toast({
        title: "Failed to create plinko game",
        description: error.message,
        variant: "destructive",
      });
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      const payload: any = {
        ...
        badgeLabel: serializeBadgeLabel(data.badgeLabel, "plinko"),
    ...
    onSuccess: () => {
      ...
      toast({ title: "Plinko game updated successfully" });
    },
    onError: (error: any) => {
      toast({
        title: "Failed to update plinko game",
        description: error.message,
        variant: "destructive",
      });
    },
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Plinko game archived successfully" });
    },
    onError: (error: any) => {
      toast({
        title: "Failed to archive plinko game",
        description: error.message,
        variant: "destructive",
      });
    },
  ...
  const unarchiveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Plinko game unarchived successfully" });
    },
    onError: (error: any) => {
      toast({
        title: "Failed to unarchive plinko game",
        description: error.message,
        variant: "destructive",
      });
    },
      ...
      <div className="space-y-4 md:space-y-6">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            >
             Ringtone Plinko Games
            </h1>
            <p className="text-sm md:text-base text-muted-foreground mt-1">
              Manage your plinko games
            </p>
          </div>
          ...
          <DialogContent className="max-w-7xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                Archived Plinko Balloon Games ({archivedCompetitions.length})
              </DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              {archivedCompetitions.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">No archived plinko balloon games</p>
                </div>
              ) : (
        ...
        <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Create New Plinko Balloon Game</DialogTitle>
            </DialogHeader>
            <CompetitionForm
        ...
        >
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Edit Plinko Balloon Game</DialogTitle>
            </DialogHeader>
            {editingCompetition && (
              <CompetitionForm
        ...
        >
          <DialogContent className="w-[90vw] max-w-sm sm:max-w-md mx-auto">
            <DialogHeader>
              <DialogTitle>Archive Plinko Balloon Game</DialogTitle>
            </DialogHeader>
            <p className="text-muted-foreground">
              Are you sure you want to archive this plink balloon game? Archived games will become inactive and won't be shown on the frontend.
            </p>
            <DialogFooter>
              <Button
                variant="outline"
