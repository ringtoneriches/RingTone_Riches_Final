...
class Logo {
    constructor(scene, offsetX, offsetY) {
        this.scene = scene;
        this.lamp = scene.addSpriteLocPos('logo', offsetX, offsetY).setScale(0.25);

        // this.fx = this.lamp.postFX.addShine(0.5, 0.2, 5);

        this.scene.tweens.add({
            targets: this.lamp,
            scaleX: { from: 0.25, to: 0.26 },
            scaleY: { from: 0.25, to: 0.26 },
            duration: 600,
            ease: 'Sine.easeInOut',
            yoyo: true,
            repeat: -1
    ...
    setOn(lampOn) {
        this.lamp.setTexture(lampOn ? 'logo' : 'logo');
    }
}
...
class Reel {
    ...
    constructor(scene, reelData, reelNumber, symbolSizeY, windowsCount, randomStartPosition, spinTime, animFrameRate) {
        ...
        this.symbolSizeY = symbolSizeY * .8;
        this.symbols = [];
        this.symbolSpites = [];
    ...
    _create() {
        this.windowOffsetY = (this.windowsCount - 1) / 2 * this.symbolSizeY * 1.2;    // 0 window offset
        this.windowPosY = this.posY + this.windowOffsetY;                       // 0 window position
        this.visibleMaxY = this.getWindowPosition(0).y + this.symbolSizeY * 0.5;
        this.visibleMinY = this.getWindowPosition(this.windowsCount - 1).y - this.symbolSizeY * 0.5;

        this.scene['reel' + this.reelNumber] = this.scene.add.image(this.posX, this.posY + 25, this.reelData.windowImage).setScale(1, .8);                              // add window image sprite
        ...
        */
        for (var si = 0; si < this.symbOrder.length; si++) {
            ...
            var symbol = new SlotSymbol(this.scene, symbSprite, symbName, posY, this.animFrameRate);
...
class LineBehavior {
    ...
    getPayLineWin(payLine) {
        ...
        return new WinData(winSymbols, payLine.freeSpins, payLine.pay, payLine.euro);
    }
...
class WinController {
    ...
    searchWinSymbols() {
        ...
        this.winLines = [];
        // IMPORTANT: winning lines were previously detected only on lines marked as selected.
        // If selection state isn't set correctly, pay can be calculated elsewhere but the
        // paylines graphics won't show. To ensure line wins always render, evaluate all lines.
    ...
    getLineWinEuro() {
        let res = false;
        this.winLines.forEach((l) => { res += l.win.euro; });
        return res;
    }
...
class PayLine {
    constructor(scene, line, pay, freeSpins, wild, euro) {
        this.scene = scene;
        this.line = line;
        this.pay = pay;
        this.euro = euro
    ...
    getWildLines() {
        ...
        while (cC.nextCombo()) {
            ...
            if ((comboSum >= minWildsCount) && (comboSum <= maxWildsCount)) {
                var p = new PayLine(this.scene, Array.from(this.line), this.pay, this.freeSpins, this.wild, this.euro);
...
class WinData {
    constructor(symbols, freeSpins, pay, euro) {
    ...
    toString() {
        return this.symbolsToString() + '\n' + 'Pay: ' + this.pay + '; FreeSpin: ' + this.freeSpins + '; Euro: ' + this.euro;
    }
}

class SlotPlayer {

    constructor(defaultCoins, defaultPoints) {
    ...
    setDefaultData() {
        ...
        this.setPointsCount(this.defaultPoints);
        this.setLevel(0);
